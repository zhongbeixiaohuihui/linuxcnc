...
/* This is the command structure.  There is one of these in shared
   ...
   这是命令结构。 共享内存中有一个，来自更高级代码的所有命令都通过它。
*/
    typedef struct emcmot_command_t {
	unsigned char head;	/* flag count for mutex detect 用于互斥检测的标志计数*/
	...
	int turn;		/* turns for circle or which rotary to unlock for a line 转动圆圈或旋转解锁线*/
	...
	double backlash;	/* amount of backlash 反弹量*/
	int id;			/* id for motion */
	int termCond;		/* termination condition 终止条件 */
	double tolerance;	/* tolerance for path deviation in CONTINUOUS mode 连续模式下路径偏差的容差 */
	...
	int home_sequence;      /* order in homing sequence 回零顺序 */
        int volatile_home;      /* joint should get unhomed when we get unhome -2 
                                   (generated by task upon estop, etc) */
	double minFerror;	/* min following error 最小跟随误差*/
	double maxFerror;	/* max following error 最大跟随误差 */
	...
	double comp_nominal, comp_forward, comp_reverse; /* compensation triplet, nominal, forward, reverse 补偿三重，标称，前进，反向*/
