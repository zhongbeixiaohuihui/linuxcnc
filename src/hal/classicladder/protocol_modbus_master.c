    ...
    if ( CurrentReq!=-1 )
    {
        ...
        if ( ModbusDebugLevel>=1 )
        {
            ...
            for( DebugFrame=0; DebugFrame<LgtAskFrame; DebugFrame++ )
            {
                if (DebugFrame==0)
                {
                    printf("Slave address-%X  ", Question[ DebugFrame]);
                }
                else
                {
                    if (DebugFrame==1)
                    {
                        printf("Function code-%X  Data-", Question[ DebugFrame]);
                    }
                    else
                    {
...
char TreatModbusMasterResponse( unsigned char * Response, int LgtResponse )
{
    ...
    if ( ModbusDebugLevel>=1 )
    {
        ...
        for( DebugFrame=0; DebugFrame<LgtResponse; DebugFrame++ )
        {
            if (DebugFrame==0)
            {
                printf("(Slave address-%X - ", Response[ DebugFrame]);
            }
            else
            {
                if (DebugFrame==1)
                {
                    printf("Function code-%X ) ", Response[ DebugFrame]);
                }
                else
                {
    ...
    if (RepOk==TRUE)
    {
        WriteVar( VAR_ERROR_BIT, 0, FALSE);
    }
    else
    {
